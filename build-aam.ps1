# Build + deploy ONLY MachineAAM (Machine_Dev is shared with other sessions, and the shared
# build-mods.ps1 has been modified by others before). ASCII-only; paths from $PSScriptRoot.
# Output -> build_aam_log.txt
$ErrorActionPreference = "Continue"

$root = $PSScriptRoot
$gameRoot = Join-Path (Split-Path $root -Parent) "Aviassembly_DEV"
$managed = Join-Path $gameRoot "Aviassembly_Data\Managed"
$ws = Join-Path $env:USERPROFILE ".workbuddy\binaries\node\workspace"
$csc = Join-Path (Join-Path (Join-Path $env:windir "Microsoft.NET\Framework64") "v4.0.30319") "csc.exe"

$logPath = Join-Path $root "build_aam_log.txt"
...
$dllPath = Join-Path $out "MachineAAM.dll"
...
if ($r.Code -ne 0) {
    Say ("!! MachineAAM BUILD FAILED (exit " + $r.Code + ")")
} else {
    Say ("-- MachineAAM OK " + (Get-Item $dllPath).Length + " bytes")
    $dstDir = Join-Path (Join-Path (Join-Path $gameRoot "mods") "MachineAAM") "code"
    if (-not (Test-Path $dstDir)) { New-Item -ItemType Directory -Force -Path $dstDir | Out-Null }
    $dst = Join-Path $dstDir "MachineAAM.dll"

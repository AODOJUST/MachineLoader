# Build + deploy ONLY MachineShop (mirrors build-aam.ps1). ASCII-only; paths from $PSScriptRoot.
# Output -> build_shop_log.txt
...
$logPath = Join-Path $root "build_shop_log.txt"
...
$dllPath = Join-Path $out "MachineShop.dll"
...
$args.Add((Join-Path $root "src\Mods\MachineShop.cs"))
...
if ($r.Code -ne 0) {
    Say ("!! MachineShop BUILD FAILED (exit " + $r.Code + ")")
} else {
    Say ("-- MachineShop OK " + (Get-Item $dllPath).Length + " bytes")
    $dstDir = Join-Path (Join-Path (Join-Path $gameRoot "mods") "MachineShop") "code"
    if (-not (Test-Path $dstDir)) { New-Item -ItemType Directory -Force -Path $dstDir | Out-Null }
    $dst = Join-Path $dstDir "MachineShop.dll"

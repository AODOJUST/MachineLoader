# Build + deploy ONLY DropTank (Machine_Dev is shared with other sessions; the shared
# build-mods.ps1 rebuilds every dll and would invalidate all other copies).
# ASCII-only; paths derived from $PSScriptRoot. Output -> build_droptank_log.txt
...
$logPath = Join-Path $root "build_droptank_log.txt"
...
$dllPath = Join-Path $out "DropTank.dll"
...
$args.Add((Join-Path $root "src\Mods\DropTank.cs"))
...
if ($r.Code -ne 0) {
    Say ("!! DropTank BUILD FAILED (exit " + $r.Code + ")")
} else {
    Say ("-- DropTank OK " + (Get-Item $dllPath).Length + " bytes")
    $dstDir = Join-Path (Join-Path (Join-Path $gameRoot "mods") "DropTank") "code"
    if (-not (Test-Path $dstDir)) { New-Item -ItemType Directory -Force -Path $dstDir | Out-Null }
    $dst = Join-Path $dstDir "DropTank.dll"

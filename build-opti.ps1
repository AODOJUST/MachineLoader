# Build + deploy ONLY OptiMod (isolated from the shared build-mods.ps1).
# ASCII-only; paths from $PSScriptRoot. Output -> build_opti_log.txt
...
$logPath = Join-Path $root "build_opti_log.txt"
...
$dllPath = Join-Path $out "OptiMod.dll"
...
$args.Add((Join-Path $root "src\Mods\OptiMod.cs"))
...
if ($r.Code -ne 0) {
    Say ("!! OptiMod BUILD FAILED (exit " + $r.Code + ")")
} else {
    Say ("-- OptiMod OK " + (Get-Item $dllPath).Length + " bytes")
    $dstDir = Join-Path (Join-Path (Join-Path $gameRoot "mods") "OptiMod") "code"
    if (-not (Test-Path $dstDir)) { New-Item -ItemType Directory -Force -Path $dstDir | Out-Null }
    $dst = Join-Path $dstDir "OptiMod.dll"
    Copy-Item $dllPath $dst -Force
    $dst2 = Join-Path (Join-Path (Join-Path $gameRoot "mods") "OptiMod") "OptiMod.dll"
    Copy-Item $dllPath $dst2 -Force

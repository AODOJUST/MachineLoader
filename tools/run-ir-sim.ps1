# Build + run the offline IR simulation (tools/ir_sim.cs).
# References the REAL compiled MachineAAM.dll (Machine.AAM.IrMath), plus
# UnityEngine.CoreModule.dll for Mathf. ASCII-only; paths from $PSScriptRoot.
# Output -> tools/ir_sim_out.txt
$ErrorActionPreference = "Continue"

$tools = $PSScriptRoot
$root = Split-Path $tools -Parent                      # Machine_Dev
...
Say ("aam  = " + (Test-Path $aamDll) + " (" + (Get-Item $aamDll).Length + " bytes)")

$exe = Join-Path $work "ir_sim.exe"

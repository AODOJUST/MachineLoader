...
# Detached self-test runner (2026-09-12). Same flow as launch-aam.ps1 but it can be
# started as an independent process so the caller is not bound by a short tool timeout.
# ASCII-only; all paths derived from $PSScriptRoot. Output -> launch_<Tag>2.txt
...
$runLog = Join-Path $root ("aam_run_log_" + $Tag + ".txt")
...
# Unity throttles to ~10fps when the window is unfocused (measured 11fps vs 42fps focused),
# which also slows scene loading. Bring the game window to the front on every heartbeat.

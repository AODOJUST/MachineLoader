...
# KillFeed self-test runner (faction-colored names).
# Writes the flag file that turns the in-mod self test on, launches the game, waits for the
# fixed done marker, slices the new part of Machine.log to a file, then kills the game.
...
$flag = Join-Path (Join-Path $gameRoot "mods\KillFeed") "_feed_test.flag"
$outPath = Join-Path $root ("launch_" + $Tag + ".txt")
$runLog = Join-Path $root ("killfeed_run_log_" + $Tag + ".txt")
...
try {
    [System.IO.File]::WriteAllText($flag, "killfeed", (New-Object System.Text.UTF8Encoding($false)))
...
while ((Get-Date) -lt $deadline) {
    ...
    $has = ($new -ne $null) -and $new.Contains("KILLFEED_SELFTEST_DONE")

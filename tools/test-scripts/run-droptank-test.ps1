param(
    [int]$DeadlineSec = 300,
    [string]$Tag = "dt",
    [string]$SaveName = "AutoSave 1"
)
# DropTank self-test runner: writes the flag file that arms the in-mod self test, launches
# the game, waits for the fixed done marker, slices the new part of Machine.log, kills the game.
...
$flag = Join-Path (Join-Path $gameRoot "mods\DropTank") "_droptank_test.flag"
$outPath = Join-Path $root ("launch_dt_" + $Tag + ".txt")
$runLog = Join-Path $root ("droptank_run_log_" + $Tag + ".txt")
...
try {
    ...
    Say ("flag written -> " + $flag + " (save=" + $SaveName + ")")
...
while ((Get-Date) -lt $deadline) {
    ...
    $has = ($new -ne $null) -and $new.Contains("DROPTANK SELFTEST DONE")

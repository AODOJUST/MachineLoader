# Build + run the offline regression for the DropTank fuel bookkeeping (tools/droptank_sim.cs).
# References the REAL compiled bin\mods\DropTank.dll (Machine.DropTank.FuelLink).
# ASCII-only; paths derived from $PSScriptRoot. Output -> tools/droptank_sim_out.txt
...
$work = Join-Path $tools "droptanksim"
if (-not (Test-Path $work)) { New-Item -ItemType Directory -Force -Path $work | Out-Null }

$outPath = Join-Path $tools "droptank_sim_out.txt"
...
$dtDll = Join-Path $root "bin\mods\DropTank.dll"
$mcDll = Join-Path $root "bin\Machine.Core.dll"
Copy-Item $coreDll $work -Force
Copy-Item $dtDll $work -Force
Copy-Item $mcDll $work -Force
Copy-Item (Join-Path $managed "netstandard.dll") $work -Force

Say ("csc  = " + $csc + " exists=" + (Test-Path $csc))
Say ("dll  = " + $dtDll + " exists=" + (Test-Path $dtDll) + " (" + (Get-Item $dtDll).Length + " bytes)")

$exe = Join-Path $work "droptank_sim.exe"
$cargs = @("/nologo", "/target:exe", ("/out:" + $exe),
           ("/r:" + (Join-Path $work "UnityEngine.CoreModule.dll")),
           ("/r:" + (Join-Path $work "DropTank.dll")),
           ("/r:" + (Join-Path $work "netstandard.dll")),
           (Join-Path $tools "droptank_sim.cs"))
